...
// I2C Slave Address Depends on state of ALT_ADDRESS, pin 12
#if ADXL_ALT_ADDRESS == 0
 const uint8_t ADDR_W = 0xA6;
 const uint8_t ADDR_R = 0xA7;
#elif ADXL_ALT_ADDRESS == 1
 const uint8_t ADDR_W = 0x3A;
 const uint8_t ADDR_R = 0x3B;
#else
#error "Please  const uint8_t ADXL_ALT_ADDRESS to be 0 or 1 in accorandance with the state of ADXL345 pin 12"
#endif
